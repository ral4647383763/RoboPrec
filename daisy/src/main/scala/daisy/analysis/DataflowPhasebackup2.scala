 ...
 */
object DataflowBackup2Phase extends DaisyPhase with RoundoffEvaluators with IntervalSubdivision with opt.CostFunctions {
  ...
  //////////** Merge two error maps by taking the maximum error for each key. */
  /////////def mergeErrorMaps(
  /////////  m1: Map[(Expr, PathCond), Rational],
  /////////  m2: Map[(Expr, PathCond), Rational]
  /////////): Map[(Expr, PathCond), Rational] = {
  /////////  val result = mutable.Map[(Expr, PathCond), Rational]()  
  /////////  // Start with everything in m1
  /////////  for ((k, v) <- m1) {
  /////////    result(k) = v
  /////////  }
  /////////  // Merge in m2
  ...
  /////////}
/////////
  //////////** Merge two range maps by taking the union of intervals for each key. */
  /////////def mergeRangeMaps(
  /////////  m1: Map[(Expr, PathCond), Interval],
  /////////  m2: Map[(Expr, PathCond), Interval]
  /////////): Map[(Expr, PathCond), Interval] = {
  /////////  val result = mutable.Map[(Expr, PathCond), Interval]()
  /////////  // Start with everything in m1
  /////////  for ((k, v) <- m1) {
  /////////    result(k) = v
  /////////  }
  ...
  /////////        result(k) = existingInterval.union(v)
  /////////      case None =>
  /////////        result(k) = v
  /////////    }
  /////////  }
