...
# 1_2 but conversion cost is removed

FONT_SIZE = 18

# Define pastel colors for numeric types
dtype_colors = {
    'pinocchio': '#bdbdbd', # light grey
...
groups_excluding_conversion = [
    {
      "label": "Intel i7-14700K",
      "num_types": ["pinocchio", "double", "float", "fixed32", "fixed32-uniform"],
      "data": {
           'Forward kinematics': [0.181, 0.130, 0.129, 0.105, 0.0911],
           'RNEA': [0.238, 0.210, 0.210, 0.225, 0.189],
           'RNEA derivatives': [1.557, 1.751, 1.642, 1.530, np.nan]
      }
    },
    {
      ...
      "data": {
           'Forward kinematics': [0.209, 0.209, 0.219, 0.200, 0.162],
           'RNEA': [0.290, 0.299, 0.407, 0.346, 0.328],
           'RNEA derivatives': [1.950, 1.951, 3.796, np.nan, np.nan]
      }
    }
]
...
## print out speedups of each datatype in each algorithm on each device, compared to double.
print("\n--- Speedups of each datatype in each algorithm on each device, compared to double ---")
for group in groups:
    print(f"\nDevice: {group['label']}")
    for algo, values in group['data'].items():
        if values is not None:
            double_value = values[0]  # Assuming the first value is always 'double'
            speedups = [double_value / v if v > 0 else np.nan for v in values]
            print(f"  {algo}: {', '.join(f'{v:.2f}' if not pd.isna(v) else 'NaN' for v in speedups)}")
        else:
...
#bottom_limit = 10**(math.floor(math.log10(min_positive_val)))
bottom_limit = 0.04
x_marker_pos = 0.045
top_limit = 4 # User's fixed top limit
        ...
        if algo in group_total_data["data"]:
            ...
            for j in range(n_types): # Iterate over all defined numeric types for the group
                ...
                if not pd.isna(value_total): # If total value is available
                   ...
                   ax.plot(bar_center_x, x_marker_pos, marker='x', color=dtype_colors.get(current_dtype, 'grey'), markersize=10, linestyle='None')
...
ax.set_title("Runtimes Across Different Platforms (Grouped by Device and Algorithm)", pad=50, fontsize=FONT_SIZE)
ax.set_yscale('log')
...
for x_pos_data, algo in algo_cluster_info:
    ...
    if not is_too_close:
        ...
        ax.text(x_pos_data, algo_label_y_pos, short_name, ha='center', va='top', transform=ax.get_xaxis_transform(), fontsize=FONT_SIZE, color='black')
...
final_legend_item_labels = [
    'pinocchio', 'double', 'float', 'fixed32', 'fixed32-uniform', 'fixed16'
]
legend_handles = []
...
if not os.path.exists("plots"):
    os.makedirs("plots")
plt.savefig("plots/runtime_nonembedded.png", dpi=600)
plt.show()
print("--- Done ---")
